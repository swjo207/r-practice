# To download
# http://race.kra.co.kr/kdb/common/Data_viewFile.jsp?fn=internet/seoul/trainer/20110106sdb3.txt&meet=1&site_flag=company

# To Browse
# http://race.kra.co.kr/kdb/common/Data_viewFile.jsp?fn=internet/seoul/trainer/20110106sdb3.txt&meet=1&site_flag=
...
## seoul url
rcresult <- sprintf('%s?fn=internet/%s/%s/%ssdb3.txt&meet=%s&site_flag=company',master, Local, Contents ,selectDates, meet)

for (i in 1:length(rcresult)){
  ex <- readLines(rcresult[i], encoding="CP949")
  FileName <- paste("./HR140_trainer/",Local,"/tmp/",Contents, "_", Local, "_", selectDates[i],".txt",sep="")
